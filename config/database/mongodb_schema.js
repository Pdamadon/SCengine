...
// =====================================================
// SELECTOR LIBRARIES COLLECTION - Reusable Selectors with Reliability
// =====================================================
const selectorLibrariesSchema = {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      ...
      properties: {
        ...
        element_type: { 
          bsonType: "string",
          enum: ["product", "price", "cart", "navigation", "search", "filter", "booking", "availability", "title", "image", "variant"]
        },
...
async function createCollections(db) {
  ...
  await db.createCollection("selector_libraries", selectorLibrariesSchema);
...
// Index creation for selector libraries
async function createSelectorIndexes(db) {
  const selectors = db.collection('selector_libraries');
  
  await selectors.createIndex({ "domain": 1, "element_type": 1 });
  await selectors.createIndex({ "domain": 1 });
  await selectors.createIndex({ "reliability_score": -1 });
  await selectors.createIndex({ "element_type": 1 });
  
  console.log("Selector library indexes created");
}
...
async function createServiceProviderIndexes(db) {
  ...
  await providers.createIndex({ "domain": 1 }, { unique: true });
