...
interface SelectorResult {
  ...
  value?: string;
  classes?: string[];
  classScores?: Array<{ class: string; score: number }>;
  alternatives?: string[];
  textContent?: string;
  tagName?: string;
  ariaLabel?: string;
...
class IntelligentSelectorGenerator {
  ...
  private bemPatterns: RegExp[];
  private semanticKeywords: string[];
  private genericClasses: string[];
   ...
   */
  generateOptimalSelector(element: Element | null, options: GenerationOptions = {}): SelectorResult {
    if (!element) {
      return { selector: null, confidence: 0, strategy: 'none' };
    }
    ...
    let bestResult: SelectorResult = { selector: null, confidence: 0, strategy: 'fallback' };
    ...
    for (const strategy of strategies) {
      try {
        ...
        this.logger?.warn('Selector generation strategy failed:', errorMessage);
      }
    }
    ...
    bestResult.metadata = {
      ...
      hasId: !!(element as HTMLElement).id,
   ...
   */
  private generateDataAttributeSelector(element: Element): SelectorResult {
    ...
    const dataAttrs: SelectorResult[] = [];
   ...
   */
  private generateSemanticClassSelector(element: Element): SelectorResult {
   ...
   */
  private generateStablePathSelector(element: Element, options: GenerationOptions = {}): SelectorResult {
   ...
   */
  private generatePositionalSelector(element: Element): SelectorResult {
   ...
   */
  private generateContentBasedSelector(element: Element): SelectorResult {
   ...
   */
  private scoreClassSemantically(className: string): number {
    let score = 0;
    ...
    let baseScore = 0.4;
    if (className.length >= 8 && className.length <= 25) {
      baseScore = 0.5;
   ...
   */
  private generateStableSegment(element: Element): PathSegment {
    const tagName = element.tagName.toLowerCase();

    // Check for ID (most stable)
    if ((element as HTMLElement).id) {
      return {
        selector: `#${(element as HTMLElement).id}`,
        isUnique: true,
        stability: 0.95,
        type: 'id',
      };
   ...
   */
  private calculatePathConfidence(path: PathSegment[]): number {
    if (path.length === 0) {
      return 0;
    }
   ...
   */
  private generateFallbackSelector(element: Element | null): SelectorResult {
    if (!element) {
      return { selector: null, confidence: 0, strategy: 'fallback' };
    }
   ...
   */
  private generateHybridSelector(element: Element | null, options: GenerationOptions = {}): SelectorResult {
    if (!element) {
      return { selector: null, confidence: 0, strategy: 'hybrid' };
    }
   ...
   */
  private generateWeightedSelector(element: Element, options: GenerationOptions = {}): SelectorResult {
    ...
    const best = candidates.sort((a, b) => (b.weightedScore || 0) - (a.weightedScore || 0))[0];
    return {
      ...best,
      originalScore: best.confidence,
      confidence: best.weightedScore || best.confidence,
    };
  }
   ...
   */
  private getContextWeights(context?: string): ContextWeights {
    const defaultWeights: ContextWeights = {
      'data-attribute': 1.0,
      'semantic-class': 0.8,
      'stable-path': 0.7,
      'positional': 0.4,
    ...
    const contextSpecificWeights: Record<string, Partial<ContextWeights>> = {
      'product.link': {
   ...
   */
  private getElementWeights(element: Element): ElementWeights {
    ...
    if ((element as HTMLElement).id || Array.from(element.attributes).some(attr => attr.name.startsWith('data-'))) {
...
export default IntelligentSelectorGenerator;