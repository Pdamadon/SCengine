...
interface Indicator {
  type: 'meta' | 'script' | 'element' | 'url' | 'api';
...
interface PlatformFingerprints {
  [platform: string]: PlatformFingerprint;
}
...
interface PlatformConfig {
  hasJsonApi: boolean;
  apiEndpoints: string[];
  commonSelectors: PlatformSelectors;
...
class PlatformDetector {
  ...
  private fingerprints: PlatformFingerprints;
   ...
   */
  detectFromUrl(url: string): URLDetectionResult {
   ...
   */
  async detectFromPage(url: string, options: DetectionOptions = {}): Promise<PageAnalysisResult> {
    if (!this.browser) {
    ...
    try {
      ...
      const analysis = await page.evaluate((fingerprints) => {
        ...
        Object.keys(fingerprints).forEach(platform => {
          ...
          platformFingerprints.indicators.forEach((indicator: any) => {
            let found = false;
            ...
            switch (indicator.type) {
              case 'meta':
                ...
                metaTags.forEach(meta => {
                  if (indicator.content.includes('name=') && (meta as HTMLMetaElement).name) {
                    const nameMatch = indicator.content.match(/name="([^"]+)"/);
                    if (nameMatch && (meta as HTMLMetaElement).name === nameMatch[1]) {
                      if (indicator.value) {
                        if (indicator.value.test && indicator.value.test((meta as HTMLMetaElement).content)) {
                          found = true;
                        }
                      } else {
                        found = true;
              ...
              case 'script':
                ...
                scripts.forEach(script => {
                  if ((script as HTMLScriptElement).src && indicator.content.test && indicator.content.test((script as HTMLScriptElement).src)) {
                    found = true;
                  }
              ...
              case 'element':
                if (document.querySelector(indicator.content as string)) {
                  found = true;
                }
                break;
      ...
      let bestMatch: PageAnalysisResult = { platform: 'unknown', confidence: 0, method: 'page_analysis' };
   ...
   */
  async detectPlatform(url: string, options: DetectionOptions = {}): Promise<URLDetectionResult | PageAnalysisResult> {
