/**
 * Reasoning Engine for AI Shopping Agent
 * Generates human-like reasoning for shopping decisions
 * Provides explanations for AI learning and training
 */
...
interface ReasoningTemplate {
  reasoning_templates: string[];
  ai_learning: string;
}

interface ActionReasoningMap {
  navigate_to_category: ReasoningTemplate;
  apply_filter: ReasoningTemplate;
  search_products: ReasoningTemplate;
  select_product: ReasoningTemplate;
  add_to_cart: ReasoningTemplate;
}
...
interface FlowContext {
  ...
  search_term?: string;
  search_element?: any;
  selection_criteria?: ExtractedAttributes;
...
class ReasoningEngine {
  ...
  private actionReasoningMap: ActionReasoningMap;
  ...
  private loadIntentPatterns(): IntentPatterns {
    return {
  ...
  private loadActionReasoningMap(): ActionReasoningMap {
    return {
  ...
  async generateReasoning(userIntent: string, sitePatterns: SitePatterns): Promise<ReasoningResult> {
    try {
  ...
  private analyzeUserIntent(userIntent: string): IntentAnalysis {
    const analysis = {
  ...
  private extractAttributes(userIntent: string): ExtractedAttributes {
  ...
  private identifyShoppingGoals(userIntent: string): string[] {
    const goals: string[] = [];
  ...
  private identifyConstraints(userIntent: string): Constraints {
  ...
  private async generateShoppingFlow(intentAnalysis: IntentAnalysis, sitePatterns: SitePatterns): Promise<FlowStep[]> {
    const flow: FlowStep[] = [];
    const { primary_intent, extracted_attributes, shopping_goals } = intentAnalysis;

    if (extracted_attributes.item_type && sitePatterns.navigation?.categories?.length && sitePatterns.navigation.categories.length > 0) {
    ...
    if (sitePatterns.products && sitePatterns.products.length > 0) {
  ...
  private createFlowStep(action: string, context: FlowContext): FlowStep {
    const reasoningConfig = this.actionReasoningMap[action as keyof ActionReasoningMap];
    if (!reasoningConfig) {
  ...
  private selectReasoningTemplate(templates: string[], context: FlowContext): string {
    const template = templates[Math.floor(Math.random() * templates.length)];
    return this.interpolateTemplate(template, context);
  }

  private interpolateTemplate(template: string, context: FlowContext): string {
    let reasoning = template;

    Object.entries(context).forEach(([key, value]) => {
      const placeholder = `{${key}}`;
      if (reasoning.includes(placeholder)) {
        reasoning = reasoning.replace(placeholder, String(value));
      }
    });
  ...
  private generateSuccessCriteria(action: string, context: FlowContext): string {
  ...
  private generateTechnicalImplementation(action: string, context: FlowContext): string {
  ...
  private generateSearchTerm(attributes: ExtractedAttributes): string {
    const terms: string[] = [];
  ...
  private calculateComplexityScore(flow: FlowStep[]): number {
