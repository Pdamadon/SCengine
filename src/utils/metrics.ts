...
interface JsonMetrics {
  counters: { [key: string]: SerializedMetric };
  gauges: { [key: string]: SerializedMetric };
  histograms: { [key: string]: SerializedMetric };
  collected_at: string;
}
...
interface MetricsSummary {
  ...
  data_points: {
    ...
    active_gauges: number;
    histogram_observations: number;
  };
  system: {
...
class MetricsCollector {
  ...
  private initializeDefaultMetrics(): void {
    ...
    this.createCounter('scraping_jobs_submitted', 'Total scraping jobs submitted', ['type', 'priority', 'status']);
    this.createHistogram('scraping_job_submission_duration_ms', 'Scraping job submission duration in milliseconds', ['type', 'priority', 'status']);
   ...
   */
  createHistogram(name: string, help: string, labels: string[] = [], buckets: number[] = [1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000]): void {
   ...
   */
  timeFunction<T>(metricName: string, fn: () => T, labels: MetricLabels = {}): T {
   ...
   */
  timed(metricName: string, labels: MetricLabels = {}) {
    const metricsInstance = this;
    return (target: any, propertyName: string, descriptor: PropertyDescriptor) => {
      const method = descriptor.value;

      descriptor.value = function(...args: any[]) {
        return metricsInstance.timeFunction(metricName, () => method.apply(this, args), labels);
      };
   ...
   */
  trackScrapingOperation(siteDomain: string, status: string, duration: number, productsFound: number = 0, category: string = ''): void {
   ...
   */
  trackDatabaseOperation(operation: string, collection: string, duration: number): void {
  ...
  /**
   * Scraping job submission tracking
   */
  trackScrapingJobSubmission(jobType: string, priority: string, status: string = 'submitted', duration?: number): void {
    this.incrementCounter('scraping_jobs_submitted', {
      type: jobType,
      priority,
      status,
    });
    ...
    if (duration !== undefined) {
      this.observeHistogram('scraping_job_submission_duration_ms', duration, {
        type: jobType,
        priority,
        status,
      });
   ...
   */
  trackQueueOperation(queueName: string, status: string, duration: number): void {
   ...
   */
  updateHealthCheckStatus(checkName: string, status: string): void {
  ...
  private serializeMetricType(metricMap: Map<string, CounterMetric | GaugeMetric>): { [key: string]: SerializedMetric } {
    const result: { [key: string]: SerializedMetric } = {};
  ...
  private serializeHistograms(): { [key: string]: SerializedMetric } {
    const result: { [key: string]: SerializedMetric } = {};
    for (const [name, histogram] of this.histograms) {
      result[name] = {
        help: histogram.help,
