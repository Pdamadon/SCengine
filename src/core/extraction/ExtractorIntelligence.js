...
class ExtractorIntelligence {
  constructor(logger, worldModel = null) {
    ...
    // Use SelectorLearningCache for persistence across sessions
    this.selectorCache = new SelectorLearningCache(logger);
    this.selectorCache.initialize().catch(err => 
      logger.warn('SelectorLearningCache initialization failed:', err.message)
    );
  ...
  async discoverFieldSelector(page, field, platform, previousAttempts = [], attemptNumber = 1) {
    ...
    const discoveredSelectors = await this.browserIntelligence.discoverSelectors(page, field);
