...
const NavigationStrategy = require('../../discovery/strategies/navigation/NavigationStrategy');
...
class ProductCatalogStrategy extends NavigationStrategy {
  constructor(logger, options = {}) {
    ...
    // Platform-specific product patterns - enhanced with sector templates
    this.productPatterns = {
      // E-commerce platforms
      shopify: {
        containers: [
          '.product-item', '.grid__item', '.card-wrapper', '.product-card-wrapper',
          // Modern Tailwind/custom patterns
          '.open-product', '[class*="open-product"]', '.grid > div', '.collection-item',
          '.product-grid-item', '.product-card', '.item-wrapper',
        ],
        links: [
          '[data-product-id] a', '.product-form a', '.product-link',
          // Enhanced link patterns
          'a[href*="/products/"]', 'a[href*="/collections/"]', '.open-product a',
          '.product-item a', '.grid__item a',
        ],
        indicators: [
          '.price', '.money', '.product-price', '[data-price]',
          // Tailwind and modern price patterns
          '[class*="price"]', '[class*="cost"]', '.open-product-cost',
      ...
      // Enhanced generic patterns with modern CSS frameworks
      modern: {
        containers: [
          // Tailwind CSS patterns
          '.grid > div', '.flex > div', '[class*="grid-cols"] > div',
          // Modern component patterns
          '.product-card', '.item-card', '.listing-item', '.collection-item',
          // Custom class patterns
          '[class*="open-product"]', '[class*="product-"]', '[class*="item-"]',
        ],
        links: [
          // Modern URL patterns
          'a[href*="/collections/"]', 'a[href*="/shop/"]', 'a[href*="/browse/"]',
          'a[href*="/products/"]', 'a[href*="/items/"]',
        ],
        indicators: [
          // Modern price indicators
          '[class*="price"]', '[class*="cost"]', '[class*="amount"]',
          '[data-price]', '[data-cost]', 'span:contains("$")', 'div:contains("$")',
        ],
      },
      generic: {
        containers: [
          '.product', '.item', '.card', '.listing', '[class*="product"]', '[class*="item"]',
          'article', 'li',
          // Additional flexible patterns
          '.grid-item', '.flex-item', '.collection-item', '.catalog-item',
        ],
        links: [
          'a[href*="/product"]', 'a[href*="/p/"]', 'a[href*="/item"]', 'a[href*="/catalogue"]',
          'a[href*="/book"]', 'a[href*="/collections/"]', 'a[href*="/browse/"]',
        ],
        indicators: [
          '.price', '[class*="price"]', '[data-price]', '$', '£', '€', '¥', '.price_color',
          '[class*="cost"]', '[class*="amount"]',
        ],
      },
    };

    // URL patterns that indicate product pages - using comprehensive sector templates
    this.productUrlPatterns = [
      ...
      // Sector template patterns for comprehensive coverage
      /\/browse\/product/i,  // Gap, Old Navy, Banana Republic
      /\/dp\//i,            // Amazon
      /\/shop\/product\//i,  // Macy's
      /\/s\//i,             // Nordstrom
      /\/product\/prd-/i,   // Kohl's
      /productpage\./i,     // H&M
      /-p/i,               // Zara
      /\/ip\//i,           // Walmart

      // Category/Collection patterns for navigation
      /\/collections\//i,   // Shopify stores
      /\/browse\//i,        // Nordstrom, Gap
      /\/c\//i,             // American Eagle
      /\/cat/i,             // Express
      /\/plp\//i,           // J.Crew
      /\.html/i,            // H&M
      /-l/i,                // Zara
      /\?cid=/i,            // Gap variations
      /\/categories\//i,    // General category
      /\/department\//i,    // Department stores
      /\/men\//i,           // Category sections
      /\/women\//i,         // Category sections
      /\/kids\//i,           // Category sections
    ];
  }
  ...
  async execute(page) {
    return await this.measureExecution(async () => {
      try {
        ...
        const initialProducts = await this.collectProductURLs(page, productAnalysis.platform);
        this.logger.info(`Found ${initialProducts.length} initial products`);

        // Step 3: Handle pagination to get ALL products
        const allProducts = await this.handlePagination(page, initialProducts, productAnalysis.platform);
   ...
   * @param {Page} page - Playwright page object
   * @param {string} detectedPlatform - Platform detected by analyzePageForProducts
   * @returns {Promise<Array>} Array of product objects
   */
  async collectProductURLs(page, detectedPlatform = 'generic') {
    return await page.evaluate((serializedData) => {
      const { patterns, detectedPlatform } = serializedData;
      ...
      // Get platform-specific selectors - use detected platform
      const platform = detectedPlatform || 'generic';
      ...
      containers.forEach((container, index) => {
        try {
          ...
          // Enhanced URL filtering - exclude obvious non-product links
          const excludePatterns = [
            '/customer_authentication/', '/account/', '/login/', '/signin/',
            '/cart/', '/checkout/', '/contact/', '/about/', '/privacy/', '/terms/',
            '/help/', '/support/', '/faq/', '/return/', '/shipping/',
            'facebook.com', 'twitter.com', 'instagram.com', 'youtube.com',
            'mailto:', 'tel:', '#',
          ];

          if (excludePatterns.some(pattern => productUrl.toLowerCase().includes(pattern))) {
            return;
          }
   ...
   */
  async handlePagination(page, initialProducts, detectedPlatform = 'generic') {
      ...
      switch (paginationType.type) {
        case 'traditional':
          if (this.config.enableTraditionalPagination) {
            const traditionalProducts = await this.handleTraditionalPagination(page, maxPages, detectedPlatform);
        ...
        case 'loadMore':
          if (this.config.enableLoadMoreButtons) {
            const loadMoreProducts = await this.handleLoadMoreButtons(page, maxPages, detectedPlatform);
        ...
        case 'infiniteScroll':
          if (this.config.enableInfiniteScroll) {
            const scrollProducts = await this.handleInfiniteScroll(page, maxPages, detectedPlatform);
   ...
   */
  async handleTraditionalPagination(page, maxPages, detectedPlatform = 'generic') {
    const products = [];
    let currentPage = 1;
    ...
    while (currentPage < maxPages) {
      try {
        ...
        const pageProducts = await this.collectProductURLs(page, detectedPlatform);
   ...
   */
  async handleLoadMoreButtons(page, maxClicks, detectedPlatform = 'generic') {
    const products = [];
    let clickCount = 0;
    ...
    while (clickCount < maxClicks) {
      try {
        ...
        const allPageProducts = await this.collectProductURLs(page, detectedPlatform);
   ...
   */
  async handleInfiniteScroll(page, maxScrolls, detectedPlatform = 'generic') {
    const products = [];
    let scrollCount = 0;
