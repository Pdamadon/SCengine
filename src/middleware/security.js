...
class SecurityService {
  ...
  createRateLimit(type = 'global') {
    ...
    return rateLimit({
      ...
      legacyHeaders: false,
      // Remove custom keyGenerator to use default IP-based key generation with IPv6 support
      handler: (req, res) => {
...
module.exports = {
  securityService,
  initializeSecurity: securityService.initializeSecurity.bind(securityService),
  jobSubmissionRateLimit: securityService.jobSubmissionRateLimit.bind(securityService),
  authRateLimit: securityService.authRateLimit.bind(securityService),
  adminOnly: securityService.adminOnly.bind(securityService),
  corsProtection: securityService.corsProtection.bind(securityService),
  validateContentType: securityService.validateContentType.bind(securityService),
};
