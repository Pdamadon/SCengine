...
import { UUID, Timestamp, Priority } from '../types/common.types';
import { ScrapingType, ScrapingJobData } from '../types/scraping.types';
...
interface JobSubmissionData {
  ...
  extract_pricing_history?: boolean;
}
...
interface JobCancellationResult {
  success: boolean;
  reason?: string;
...
class ScrapingJobService {
   ...
   */
  async submitJob(jobData: JobSubmissionData): Promise<JobSubmissionResult> {
    const jobId = uuidv4();
    ...
    const jobRecord: JobRecord = {
      ...
      estimated_completion: new Date(), // Will be set below
    };
    ...
    try {
      ...
      if (this.db) {
        await this.db.collection(this.jobsCollection).insertOne(jobRecord as any);
      }
      ...
      throw new Error(`Failed to create scraping job: ${errorMessage}`);
    }
  }
   ...
   */
  async getJobStatus(jobId: UUID): Promise<JobStatusResponse | null> {
    try {
      if (!this.db) {
   ...
   * Get job results
   */
  async getJobResults(jobId: UUID, format: string = 'json'): Promise<JobResultsResponse | null> {
    try {
      if (!this.db) {
        throw new Error('Database not available');
      }
   ...
   * Cancel a job
   */
  async cancelJob(jobId: UUID): Promise<JobCancellationResult> {
    try {
      if (!this.db) {
        throw new Error('Database not available');
      }
   ...
   */
  private estimateJobDuration(jobData: JobSubmissionData): number {
   ...
   */
  private async addToQueue(jobRecord: JobRecord): Promise<number> {
    try {
      ...
      const priorityPositions: Record<Priority, number> = {
        urgent: 1,
        high: 5,
        normal: 10,
        low: 20,
   ...
   */
  private async removeFromQueue(jobId: UUID): Promise<boolean> {
    try {
      // Ensure queue manager is initialized
      if (!queueManager.isInitialized) {
        await queueManager.initialize();
   ...
   */
  private async formatResults(data: any, format: string): Promise<any> {
    switch (format) {
      case 'csv':
...
export default ScrapingJobService;