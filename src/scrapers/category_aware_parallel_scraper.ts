...
class CategoryAwareParallelScraper {
  ...
  private categoryResults: Map<string, CategoryStats>;
  ...
  async scrapeWithCategoryIntelligence(): Promise<AggregatedResult> {
  ...
  async extractCategorizedProducts(intelligence: ComprehensiveIntel): Promise<Map<string, CategoryData>> {
    const categorizedProducts = new Map<string, CategoryData>();
    ...
    if (intelligence.exploration && intelligence.exploration.total_products_found > 0) {
      ...
      const generalCategoryData: CategoryData = {
  ...
  async discoverProductsWithIntelligence(intelligence: ComprehensiveIntel): Promise<string[]> {
...
`;
  ...
  async processCategoriesInParallel(categoryResults: Map<string, CategoryData>): Promise<void> {
    const categoryBatches: CategoryBatchData[] = [];
  ...
  async processCategoryBatch(categoryBatchData: CategoryBatchData): Promise<void> {
...
`;
      ...
      child.on('close', (code) => {
        ...
        if (code === 0) {
          try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            logger.error(`❌ Category batch ${batchIndex + 1} result parsing failed: ${errorMessage}`);
            this.failedBatches.push({ batchIndex, categoryName, error: errorMessage, stdout, stderr });
          }
        } else {
  ...
  async fallbackToBasicDiscovery(): Promise<AggregatedResult> {
    logger.warn('🔄 Falling back to basic discovery method...');
    const { FullSiteParallelScraper } = await import('./full_site_parallel_scraper');
  ...
  updateProgress(completed: number, total: number): void {
    ...
    logProgress(completed, total, `| Products: ${this.totalProductsScraped} | Rate: ${rate.toFixed(2)}/sec | ETA: ${eta.toFixed(1)}min | Categories: ${this.categoryResults.size}`);
  }

  aggregateResultsWithCategories(totalTime: number, totalFound: number, intelligence: ComprehensiveIntel): AggregatedResult {
...
async function runCategoryAwareScraping(): Promise<boolean> {
...
export { CategoryAwareParallelScraper };
