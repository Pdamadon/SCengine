...
import { ParallelScraper } from './parallel_scraper';
import GlasswingScraper from './GlasswingScraper';
import { Logger } from '../types/common.types';

// Simple logger
const logger: Logger = {
  info: (...args: any[]) => console.log(`[COORDINATOR] ${new Date().toISOString()}:`, ...args),
  error: (...args: any[]) => console.error(`[COORDINATOR] ${new Date().toISOString()}:`, ...args),
  warn: (...args: any[]) => console.warn(`[COORDINATOR] ${new Date().toISOString()}:`, ...args),
  debug: (...args: any[]) => console.log(`[COORDINATOR] ${new Date().toISOString()}:`, ...args),
};
...
class ParallelProductScraper {
  ...
  private parallelScraper: ParallelScraper;
  private discoveryScaper: GlasswingScraper;
  ...
  async scrapeCollectionsInParallel(collections: Collection[]): Promise<CollectionResults> {
  ...
  async scrapeProductsInParallel(collectionUrl: string, maxProducts: number | null = null, productsPerWorker: number = 10): Promise<AggregatedResult> {
    ...
    const allProductUrls = discoveryResult.productAnalysis.map((p: any) => p.url).filter((url: string) => url && !url.includes('error'));
    ...
    return new Promise((resolve) => {
      ...
      this.parallelScraper.onAllJobsComplete = () => {
        ...
        this.parallelScraper.completedJobs.forEach((job: Job) => {
  ...
  async scrapeEntireSiteInParallel(): Promise<AggregatedResult> {
...
// Example usage
async function runParallelProductScraping(): Promise<void> {
